...
StereoBuffer16 DecodeADPCM(const u8* const data, const size_t sample_count,
    ...
    const size_t ret_size{sample_count % 2 == 0 ? sample_count
                                                : sample_count + 1}; // Ensure multiple of two.
    StereoBuffer16 ret{ret_size};

    int yn1{state.yn1}, yn2{state.yn2};

    const size_t NUM_FRAMES{(sample_count + (SAMPLES_PER_FRAME - 1)) /
                            SAMPLES_PER_FRAME}; // Round up.
    for (size_t framei{}; framei < NUM_FRAMES; framei++) {
        ...
        for (size_t i{}; i < SAMPLES_PER_FRAME && outputi < sample_count; i += 2) {
