...
double TimeStretcher::CalculateCurrentRatio() {
    ...
    const double expected_time{static_cast<double>(impl->samples_queued) /
...
double TimeStretcher::CorrectForUnderAndOverflow(double ratio, std::size_t sample_delay) const {
    const std::size_t min_sample_delay{static_cast<std::size_t>(MIN_DELAY_TIME * impl->sample_rate)};
    const std::size_t max_sample_delay{static_cast<std::size_t>(MAX_DELAY_TIME * impl->sample_rate)};
