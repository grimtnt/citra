    ...
    ui(new Ui::ControlPanel)
{
    ...
    ui->network_state->setValue(Settings::values.n_state);
    connect(ui->power_adapter_connected, &QCheckBox::stateChanged, this, &ControlPanel::OnAdapterConnectedChanged);
    connect(ui->power_battery_charging, &QCheckBox::stateChanged, this, &ControlPanel::OnBatteryChargingChanged);
    connect(ui->power_battery_level, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &ControlPanel::OnBatteryLevelChanged);
    connect(ui->network_wifi_status, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &ControlPanel::OnWifiStatusChanged);
    connect(ui->network_link_level, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this, &ControlPanel::OnWifiLinkLevelChanged);
    connect(ui->network_state, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this, &ControlPanel::OnNetworkStateChanged);
...
void ControlPanel::OnWifiLinkLevelChanged() {
    Settings::values.n_wifi_link_level =
            static_cast<u8>(ui->network_link_level->value());
    SharedPage::shared_page.wifi_link_level =
            static_cast<u8>(ui->network_link_level->value());
}

void ControlPanel::OnNetworkStateChanged() {
    Settings::values.n_state =
            static_cast<u8>(ui->network_state->value());
    SharedPage::shared_page.network_state =
            static_cast<u8>(ui->network_state->value());
}
