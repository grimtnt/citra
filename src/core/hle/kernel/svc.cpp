...
static ResultCode WaitSynchronization1(Handle handle, s64 nano_seconds) {
    ...
    NGLOG_TRACE(Kernel_SVC, "called handle=0x{:08X}({}:{}), nanoseconds=%lld", handle,
...
static void OutputDebugString(VAddr address, int len) {
    ...
    NGLOG_DEBUG(Debug_Emulated, "%.*s", len, string.data());
}
...
static ResultCode CreateMutex(Handle* out_handle, u32 initial_locked) {
    ...
    NGLOG_TRACE(Kernel_SVC, "called initial_locked={} : created handle=0x{:08X}", initial_locked,
                *out_handle);
