...
void NDM_U::EnterExclusiveState(Kernel::HLERequestContext& ctx) {
    ...
    LOG_WARNING(Service_NDM, "(STUBBED) called, exclusive_state=0x{:08X}",
...
void NDM_U::SuspendDaemons(Kernel::HLERequestContext& ctx) {
    ...
    IPC::RequestBuilder rb = rp.MakeBuilder(1, 0);
    rb.Push(RESULT_SUCCESS);
    LOG_WARNING(Service_NDM, "(STUBBED) called, bit_mask=0x{:08X}", bit_mask);
}
...
void NDM_U::SuspendScheduler(Kernel::HLERequestContext& ctx) {
    ...
    IPC::RequestBuilder rb = rp.MakeBuilder(1, 0);
    rb.Push(RESULT_SUCCESS);
    LOG_WARNING(Service_NDM, "(STUBBED) called, perform_in_background={}", perform_in_background);
}
...
void NDM_U::QueryStatus(Kernel::HLERequestContext& ctx) {
    ...
    LOG_WARNING(Service_NDM, "(STUBBED) called, daemon=0x{:02X}", daemon);
}
...
void NDM_U::SetScanInterval(Kernel::HLERequestContext& ctx) {
    ...
    IPC::RequestBuilder rb = rp.MakeBuilder(1, 0);
    rb.Push(RESULT_SUCCESS);
    LOG_WARNING(Service_NDM, "(STUBBED) called, scan_interval=0x{:08X}", scan_interval);
}
...
void NDM_U::SetRetryInterval(Kernel::HLERequestContext& ctx) {
    ...
    IPC::RequestBuilder rb = rp.MakeBuilder(1, 0);
    rb.Push(RESULT_SUCCESS);
    LOG_WARNING(Service_NDM, "(STUBBED) called, retry_interval=0x{:08X}", retry_interval);
}
